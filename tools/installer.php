...
class Installer
{
    ...
    private function copyMimAutocomplete(){
        ...
        $cli = readline('What is your terminal type? (BASH/zsh): ');

        if(!$cli)
            $cli = 'B';

        $cli = substr(strtolower($cli), 0, 1);
        ...
        if($cli === 'b'){
            ...
            $nl = PHP_EOL;
            $tx = $nl . $nl;
            $tx.= '# generated by mim installer' . $nl;
            $tx.= 'autoload -U +X compinit && compinit' . $nl;
            $tx.= 'autoload -U +X bashcompinit && bashcompinit' . $nl;
            $tx.= 'source ' . $this->here . '/etc/bash/au.sh';

            $zshrc = chop($_SERVER['HOME'], '/') . '/.zshrc';
            $f = fopen($zshrc, 'a');
            fwrite($f, $tx);
            fclose($f);
        }
